...
def sample_diffusion_ligand(model, data, num_samples, batch_size=16, device='cuda:0',
                            ...
                            sample_num_atoms='prior', normal_net_cond=None, relative_net_cond=None, cond_dim=128):
    assert normal_net_cond is not None and relative_net_cond is not None
...
if __name__ == '__main__':
    ...
    normal_net_cond = Normal_BAPNet(ckpt_path=train_config.normal_net_cond.ckpt_path,
                                    hidden_nf=train_config.normal_net_cond.hidden_dim).to(args.device)
    relative_net_cond = Relative_BAPNet(ckpt_path=train_config.relative_net_cond.ckpt_path,
                                        hidden_nf=train_config.relative_net_cond.hidden_dim).to(args.device)
