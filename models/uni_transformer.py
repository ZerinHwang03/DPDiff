...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def forward(self, h, h_uncond, x, mask_ligand, batch, return_all=False, fix_x=False):
        guidance_scale = 0.75
        all_x = [x]
        all_h = [h]
        all_h_uncond = [h_uncond]
        ...
        for b_idx in range(self.num_blocks):
            ...
            for l_idx, layer in enumerate(self.base_block):
                h_uncond, x = layer(h_uncond, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)
                h, x = layer(h, x, edge_type, edge_index, mask_ligand, e_w=e_w, fix_x=fix_x)

            all_x.append(x)
            all_h.append(h)
            all_h_uncond.append(h_uncond)
        
        h = h_uncond + guidance_scale * (h - h_uncond)
        outputs = {'x': x, 'h': h}
        if return_all:
