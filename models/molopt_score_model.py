...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        ## [BF] self-condition hbap fusion
        self.cond_dim = config.cond_dim
        self.emb_mlp = nn.Linear(emb_dim + self.cond_dim * 2, emb_dim)

    # forward process is provided to x0 prediction during the training process
    # the inputs are disturbed positions and atom types
    def forward(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand, time_step=None, return_all=False, fix_x=False,
                normal_hbap_protein=None, normal_hbap_ligand=None,
                relative_hbap_protein=None, relative_hbap_ligand=None):
        ...
        ## [BF] apply feature fusion here
        if normal_hbap_protein is None:
            normal_hbap_protein = torch.zeros([h_protein.shape[0], self.cond_dim]).to(h_protein.device)
        if normal_hbap_ligand is None:
            normal_hbap_ligand = torch.zeros([init_ligand_h.shape[0], self.cond_dim]).to(init_ligand_h.device)

        ## [BF] apply feature fusion here
        if relative_hbap_protein is None:
            relative_hbap_protein = torch.zeros([h_protein.shape[0], self.cond_dim]).to(h_protein.device)
        if relative_hbap_ligand is None:
            relative_hbap_ligand = torch.zeros([init_ligand_h.shape[0], self.cond_dim]).to(init_ligand_h.device)

        h_protein = self.emb_mlp(torch.cat([h_protein, normal_hbap_protein, relative_hbap_protein], dim=1))
        init_ligand_h = self.emb_mlp(torch.cat([init_ligand_h, normal_hbap_ligand, relative_hbap_ligand], dim=1))

        h_protein_uncond = self.emb_mlp(torch.cat([h_protein, torch.zeros_like(normal_hbap_protein), torch.zeros_like(relative_hbap_protein)], dim=1))
        init_ligand_h_uncond = self.emb_mlp(torch.cat([init_ligand_h, torch.zeros_like(normal_hbap_ligand), torch.zeros_like(relative_hbap_ligand)], dim=1))
        ...
        if self.config.node_indicator:
            ...
            h_protein_uncond = torch.cat([h_protein_uncond, torch.zeros(len(h_protein_uncond), 1).to(h_protein_uncond)], -1)
            init_ligand_h_uncond = torch.cat([init_ligand_h_uncond, torch.ones(len(init_ligand_h_uncond), 1).to(h_protein_uncond)], -1)

        ## [BF] get hbap_all
        # h_all, pos_all, batch_all, mask_ligand, hbap_all = compose_context(
        h_all, h_all_uncond,pos_all, batch_all, mask_ligand, _ = compose_context(
            h_protein=h_protein,
            h_ligand=init_ligand_h,
            h_protein_uncond=h_protein_uncond,
            h_ligand_uncond=init_ligand_h_uncond,
        ...
        outputs = self.refine_net(h_all, h_all_uncond, pos_all, mask_ligand, batch_all, return_all=return_all, fix_x=fix_x)
    ...
    def get_diffusion_loss(
            self, normal_net_cond, relative_net_cond, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v,
            batch_ligand, time_step=None
    ):
        ...
        normal_hbap_ligand = None
        normal_hbap_protein = None
        relative_hbap_ligand = None
        relative_hbap_protein = None

        gt_protein_v = protein_v
        gt_protein_pos = gt_protein_pos
        ...
        if self.model_mean_type == 'noise':
            ...
            ## [BF] the pos is shifted to CoM already

            gt_ligand_v = ligand_v
            gt_ligand_pos = gt_ligand_pos
            gt_lig_a_h = gt_ligand_v

            # Normal HBAP
            normal_hbap_ligand, normal_hbap_protein = normal_net_cond.extract_features(gt_ligand_pos, gt_protein_pos, gt_lig_a_h, gt_protein_a_h, gt_protein_r_h, batch_ligand, batch_protein)
            normal_hbap_ligand = normal_hbap_ligand * ligand_hbap_mask
            normal_hbap_protein = normal_hbap_protein * protein_hbap_mask

            # Relative HBAP
            relative_hbap_ligand, relative_hbap_protein = relative_net_cond.extract_features(gt_ligand_pos, gt_protein_pos, gt_lig_a_h, gt_protein_a_h, gt_protein_r_h, batch_ligand, batch_protein)
            relative_hbap_ligand = relative_hbap_ligand * ligand_hbap_mask
            relative_hbap_protein = relative_hbap_protein * protein_hbap_mask
        else:
            raise ValueError
        ...
        preds = self(
            ...
            normal_hbap_protein=normal_hbap_protein,
            normal_hbap_ligand=normal_hbap_ligand,
            relative_hbap_protein=relative_hbap_protein,
            relative_hbap_ligand=relative_hbap_ligand,
        )
    ...
    ## [BF] introduce self-conditioning
    @torch.no_grad()
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, init_ligand_v, batch_ligand,
                         num_steps=None, center_pos_mode=None, pos_only=False, normal_net_cond=None, relative_net_cond=None, cond_dim=128):
        ...
        ## [BF] first init of hbap. in timestep T, hbap is zero-tensor
        normal_hbap_protein = None
        normal_hbap_ligand = None
        relative_hbap_protein = None
        relative_hbap_ligand = None
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            ## [BF] extract hbap from x0_from_e for next step
            normal_hbap_ligand, normal_hbap_protein = normal_net_cond.extract_features(pred_ligand_pos, protein_pos, pred_lig_a_h, gt_protein_a_h, gt_protein_r_h, batch_ligand, batch_protein)
            normal_hbap_ligand, normal_hbap_protein = normal_hbap_ligand.detach(), normal_hbap_protein.detach()

            relative_hbap_ligand, relative_hbap_protein = relative_net_cond.extract_features(pred_ligand_pos, protein_pos, pred_lig_a_h, gt_protein_a_h, gt_protein_r_h, batch_ligand, batch_protein)
            relative_hbap_ligand, relative_hbap_protein = relative_hbap_ligand.detach(), relative_hbap_protein.detach()
