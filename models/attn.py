...
class RetAugmentationLinearAttention(nn.Module):
    ...
    def forward(self, h, h_retrieved):
        ...
        attn = self.linear_attn(h)  # [b, n_self, in_dim] -> [b, n_self, d]
        attn = torch.softmax(attn, dim=-1)  # [b, n_self, d] -> [b, n_self, d]
        assert h.shape[-1] == h_retrieved.shape[-1], "h.shape[-1] != h_retrieved.shape[-1]"
        kv_in = torch.cat([h, h_retrieved], dim=1)  # [b, n_self + n_ret, in_dim]
        k = self.to_k(kv_in)  # [b, n_self + n_ret, in_dim] -> [b, n_self + n_ret, d]
        v = self.to_v(kv_in)  # [b, n_self + n_ret, in_dim] -> [b, n_self + n_ret, d]
        f = torch.bmm(k.permute(0, 2, 1), v)  # [b, d, n_self + n_ret] x [b, n_self + n_ret, d] -> [b, d, d]

        h_aug = torch.bmm(attn, f)  # [bm, n_self, d] x [b, d, d] -> [b, n_self, d]
